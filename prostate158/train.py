...
from .data import import_dataset, DataLoader, num_workers, segmentation_dataloaders
...
class SegmentationTrainer(monai.engines.SupervisedTrainer): 
    ...
    def __init__(self, 
        ...
        if train_files is None or val_files is None:
            # Comportamento antigo se os dados não forem passados diretamente
            train_loader, val_loader = segmentation_dataloaders(
                config=config, train=True, valid=True, test=False
            )
        else:
            # Novo comportamento para validação cruzada
            ...
            train_loader = DataLoader(
                train_ds, batch_size=config.data.batch_size, num_workers=2, shuffle=True, persistent_workers=True
            )
